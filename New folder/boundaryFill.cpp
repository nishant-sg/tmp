...
// For 8 Flood Fill Algorithm

void flood2(int x, int y,struct Color old_col,struct Color new_col)
{
struct Color color;
color=getPixelColor(x,y);
printf("Filling Color\n" );
...
// // recursive call for left pixel fill
flood(x, y - 1,old_col,new_col);
// recursive call for diagonal fill
flood(x-1, y - 1,old_col,new_col);
// recursive call for diagonal fill
flood(x +1 , y - 1,old_col,new_col);
// recursive call for diagonal fill
flood(x -1, y + 1,old_col,new_col);
...
draw_line(200,400,200,100);
draw_line(200,400,200,400);
...
flood2(125,125,old_col,new_col);
glFlush();
}
